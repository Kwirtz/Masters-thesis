...
dataset = pd.get_dummies(pd.read_csv("bank-full.csv",delimiter=";"))
...
def NeuralNetwork():
    ...
    for train_index,test_index in cv.split(X):
        ...
        model.fit(X[train_index],y[train_index],epochs=64, batch_size=32, verbose = 0)
...
clfRF = RandomForestClassifier(n_estimators = 128, criterion ="gini", oob_score = True, random_state = 0, max_features = "sqrt")
clfSVM = svm.LinearSVC(C = 100)
