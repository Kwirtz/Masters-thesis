...
dataset = shuffle(pd.get_dummies(pd.read_csv("sonar.all-data",delimiter=",", header = None)))
...
def NeuralNetwork():
    ...
    for train_index,test_index in cv.split(X):
        ...
        model.fit(X[train_index],y[train_index],epochs=64, batch_size=4, verbose = 0)
...
clfRF = RandomForestClassifier(n_estimators = 128, criterion ="gini", oob_score = False, random_state = 12345, max_features = "sqrt")
clfSVM = svm.LinearSVC(C = 10)
clfSVMKernel = svm.SVC(kernel='rbf', C = 10, gamma = 1 )
