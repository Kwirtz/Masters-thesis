...
def NeuralNetwork():
    ...
    for train_index,test_index in cv.split(X):
        ...
        model.fit(X[train_index],to_categorical(y[train_index])[:,1:],epochs=64, batch_size=32, verbose = 0)
...
clfSVMKernel = svm.SVC(kernel='rbf', C= 1, gamma = 0.01 )
