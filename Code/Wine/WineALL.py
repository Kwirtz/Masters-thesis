...
def NeuralNetwork():
    ...
    for train_index,test_index in cv.split(X):
        ...
        model.fit(X[train_index],to_categorical(y[train_index]),epochs=200, batch_size=4, verbose = 1000)
...
clfSVMKernel = svm.SVC(kernel='rbf', C= 10, gamma = 0.01 )
